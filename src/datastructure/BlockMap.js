(function () {
    jsd.datastructure.BlockMap = Object.subClass({
        name:'jsd.datastructure.BlockMap',
        ...
        ctor: function (fileSize, resourceId) {
            ...
            this.numOfChunksInBlock = jsd.config.BLOCK_SIZE / jsd.config.CHUNK_SIZE;
            this.fileSize = fileSize;
            this.numOfBlocks = Math.ceil(this.fileSize / jsd.config.BLOCK_SIZE);
            this.numOfChunks = Math.ceil(this.fileSize / jsd.config.CHUNK_SIZE);
            this.numOfVerifiedBlocks = 0;
            this.firstMissingBlock = 0;
            this.sizeOfLastBlock = fileSize % jsd.config.BLOCK_SIZE;
            this.availabilityMap = new jsd.datastructure.AvailabilityMap(this.numOfBlocks);
            this.fs = false;
            this._registerEvents();
            if (jsd.config.USE_FS) {
                this.lruMap = new jsd.datastructure.LRU(Math.floor(jsd.config.CACHE_SIZE/jsd.config.BLOCK_SIZE)
                    ,this._lruRemoveCb);
...
//        },
        ...
        getChunkIdsOfBlock:function (blockId) {
            if (blockId >= this.numOfBlocks)
                return [];
            var numOfChunksInThisBlock = (blockId == this.numOfBlocks - 1) ? Math.ceil(this.sizeOfLastBlock / jsd.config.CHUNK_SIZE) : this.numOfChunksInBlock;
            var chunkIds = [];
        ...
        getChunk:function (chunkId,cb) {
            ...
            if (this.privateBlockMap[indices.block]) {
                ...
                if (jsd.config.USE_FS) {
                    ...
                    if(retval == true) {
                        //read from fs
                        var blockOffset = indices.block*jsd.config.BLOCK_SIZE;
                        jsd.data.FSio.read(this.resourceId,blockOffset,blockOffset+jsd.config.BLOCK_SIZE
                            ,function(succ,data){
                                if(succ){
        ...
        setChunk:function (chunkId, chunkData) {
            var indices = this.calcBlockIdChunkOffset(chunkId);
            if (!this.privateBlockMap[indices.block]) {
                var blockSize = (indices.block == this.numOfBlocks - 1) ? this.sizeOfLastBlock : (jsd.config.BLOCK_SIZE);
                this.privateBlockMap[indices.block] = new jsd.core.Block(blockSize);
            }
        ...
        getBlockIndex:function(index) {
            return Math.ceil(index/jsd.config.BLOCK_SIZE);
        },
        ...
        getConsecutiveBuffer: function(startBlock) {
            ...
            var array = new Uint8Array(jsd.config.BLOCK_SIZE*numOfBlocks);
            for (var i = 0; i < numOfBlocks; ++i) {
                array.set(this.getBlock(i), i * jsd.config.BLOCK_SIZE);
            }
            return array;
        },
        ...
        verifyBlock: function (blockId) {
            if (this.privateBlockMap[blockId]) {
                if(this.privateBlockMap[blockId].verifyBlock() == 1){
                    ...
                    var thi$ = this;
                    if (jsd.config.USE_FS && this.fs) {
                        jsd.data.FSio.write(thi$.resourceId,new Blob([thi$.getBlock(blockId)]),blockId*jsd.config.BLOCK_SIZE,function(succ){
                            if(succ) {
                                //adding the block to the lruMap
                                thi$.lruMap.set(blockId,thi$.privateBlockMap[blockId]);
                                logger.debug("successfully wrote block " + blockId + " to filesystem ");
                            } else {
                                logger.warn("couldn't write block " + blockId + " to filesystem");
                            }
                        });
                    }
        ...
        initiateFromLocalData: function(cb,blockId,endBlockId) {
            ...
            if(!endBlockId && endBlockId != 0) {
                jsd.data.FSio.getResourceDetails(this.resourceId,function(succ,details){
                   var endBlockId = Math.floor(details.size/jsd.config.BLOCK_SIZE);
                   thi$.initiateFromLocalData(cb,blockId,endBlockId);
                });
            } else {
                jsd.data.FSio.read(thi$.resourceId,blockId*jsd.config.BLOCK_SIZE,(blockId+1)*jsd.config.BLOCK_SIZE,function(succ,data){
                    if (succ) { //the block is read now we need to verify it against corruptions
                        if (thi$.initiateBlockFromLocalData(blockId,data)) {
                            logger.log("successfully initiated block " + blockId + " from filesystem " + Date.now());
                        } else {
                            logger.log("couldnt verify block " + blockId + " from filesystem " + Date.now());
                        }
                        if(blockId < endBlockId)
                        {
                            ...
                            logger.info("finished initiating from filesystem");
                            cb(true);
                        }
                    } else {
                        logger.warn("couldn't initiate block " + blockId + " from filesystem");
                    }
                });
            }
        ...
        initiateBlockFromLocalData: function(blockId, data) {
            var blockSize = (blockId == this.numOfBlocks - 1) ? this.sizeOfLastBlock : (jsd.config.BLOCK_SIZE);
            this.privateBlockMap[blockId] = new jsd.core.Block(blockSize);
            for(var i=0; i<Math.ceil(data.length/jsd.config.CHUNK_SIZE);++i)
            ...
            if(this.privateBlockMap[blockId].verifyBlock(data)) {
                ...
                for(var i=0; i<Math.ceil(data.length/jsd.config.CHUNK_SIZE);++i)
                    radio('jsd_received_fs_chunk').broadcast(blockId*this.numOfChunksInBlock+i,this.resourceId);
                if(this.isFull()) radio('transferFinishedEvent').broadcast(this); //only call when a new block is completed
                this.availabilityMap.set(blockId);
                return true;
            } else {
        ...
        saveLocally: function() {
            ...
            if (jsd.config.USE_FS) {
                logger.info('saving file via file system api ')
                this._saveLocallyUsingFSio();
            } else {
                this._saveLocally();
                logger.info('saving file without file system api ')
            }
        },

        changeResourceId:function(newResourceId,cb) {
            var thi$ = this;
            jsd.data.FSio.renameResource(this.resourceId,newResourceId,function(succ){
                if(succ) {
        ...
        _initiateFileSystem:function(){
            ...
            jsd.data.FSio.requestQuota(this.fileSize,function(succ,freeSpace){
                if(succ){ //success to create filesystem
                    jsd.data.FSio.isExist(resourceId,function(succ){
                        if(succ){ //file exists
                            logger.info("Resource " + resourceId + " exists already in the filesystem.");
                            thi$.fs = true;
                            thi$.initiateFromLocalData(function(succ){
                                if(succ){ //initiated from local data
                                    var fileSizeLeft = (thi$.numOfBlocks - thi$.numOfVerifiedBlocks)*jsd.config.BLOCK_SIZE;
                                    if(freeSpace > fileSizeLeft){ //we hav enough space
                                        ...
                                        logger.warn("We don't have enough space for the file");
                                        jsd.config.USE_FS = false;
                                        radio('resourceInit').broadcast(thi$.metadata);
                                        radio('filesystemInitiated').broadcast();
                                    ...
                                    logger.warn("There was a problem initiating from filesystem");
                                }
                            },0);
                            //TODO: verify that freeSpace > amount of file left to download
                        }else{ //file doesn't exist
                            logger.info("Resource " + resourceId + " doesn't exist in the filesystem.");
                            if(freeSpace > thi$.fileSize){ //we have enough space let's create the resource
                                jsd.data.FSio.createResource(resourceId,function(succ){
                                    if(succ){
                                        thi$.fs = true;
                                    }else{
                                        thi$.fs = false;
                                ...
                                jsd.data.FSio.removeRootDir(function(){
                                    jsd.data.FSio.queryQuota(function(succ,usage,quota){
                                        if(quota - usage > thi$.fileSize){ //we have enough space let's create the resource
                                            ...
                                            jsd.data.FSio.requestQuota(thi$.fileSize + usage,function(succ,freeSpace){
                                                if(succ){
                                                    if(freeSpace > thi$.fileSize){ //we have enough space let's create the resource
                                                        jsd.data.FSio.createResource(resourceId,function(succ){
                                                            if(succ){
                    ...
                    logger.warn("failed to create the filesystem");
                    jsd.config.USE_FS = false;
                    radio('resourceInit').broadcast(thi$.metadata);
                    radio('filesystemInitiated').broadcast();
                }
        ...
        _saveLocallyUsingFSio:function(){
            logger.debug("saveLocally called");
            var thi$ = this;
            jsd.data.FSio.createObjectURL(this.resourceId,function(succ,url){
                if(!succ) return;
                ...
                //jsd.data.FSio.removeResource(name);
            })
        },
