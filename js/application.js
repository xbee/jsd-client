...
var addSessionCallbacks, removeSessionCallbacks, addCallCallbacks, removeCallCallbacks,
    getUserMedia, resetSessionUI, resetCallUI, callCleanup, hasUserAudio, hasUserVideo,
...
function session_onIncoming(event) {
    ...
    addCallCallbacks(id);
    $('#' + id + ' .callStatus').html('Incoming call from ' + caller);
...
function call_onConnected(event) {
    ...
    $('#' + id + ' .callStatus').html('In call with ' + getRemoteParty(this));
    ...
    if (users[id]) {
        if (users[id].session && users[id].session.getStatus() === 'connected') {
            ...
            if (users[id].isIncomingCall && users[id].call) {
                ...
                toList = id === 'user1' ? users.user2.userId : users.user1.userId;
                mediatypes = 'audio,video';

                // Create Call
                users[id].call = users[id].session.createCall(toList, mediatypes); //TODO allow audio call option
...
function getUserMedia(id, forceNew) {
...
function initUI() {
    ...
    $('.sessionConnect,.sessionDisconnect,.callDisconnect,.callReject').tooltip({
        items: '.sessionConnect,.sessionDisconnect,.callDisconnect,.callReject',
        content: function () {
...
$(document).ready(function () {
    bob = new User('sip:bob@example.com', 'user1');
    alice = new User('sip:alice@example.com', 'user2');
    users = {user1: bob, user2: alice};
    initUI();
});
