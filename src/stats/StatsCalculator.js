(function () {
    ...
    jsd.stats.StatsCalculator = Object.subClass({
        ...
        addP2PRecv:function () {
            this.Total_Recv_P2P += jsd.config.CHUNK_SIZE;
        },
        addP2PWaste:function () {
            this.numOfP2PWasteChunks++;
            this.Total_Waste_P2P += jsd.config.CHUNK_SIZE;
        },
        addP2PSent:function () {
            this.Total_Sent_P2P += jsd.config.CHUNK_SIZE;
        },
        addHTTP:function () {
            this.Total_Recv_HTTP += jsd.config.CHUNK_SIZE;
        },
        ...
        calc_avg:function (forceCalc) {
            ...
            if (this.statsTimestamp == 0) {
                ...
                radio('jsd_state_updated').broadcast(this);
        ...
        registerEvents:function () {
            ...
            radio('jsd_received_fs_chunk').subscribe([function(chunkId,swarmId){
                this.Total_loaded_FS += jsd.config.CHUNK_SIZE;
            },this]);

            radio('jsd_received_http_chunk').subscribe(function (chunkId, swarmId) {
                //thi$.Total_Recv_HTTP += jsd.config.CHUNK_SIZE;
                //thi$.calc_avg(false);
            });

            radio('jsd_pending_http_chunk').subscribe(function (chunkId, swarmId) {

            });

            radio('jsd_waste_http_chunk').subscribe(function (chunkId, swarmId, originId) {
                thi$.Total_Waste_P2P += jsd.config.CHUNK_SIZE;
            });

            radio('jsd_new_p2p_chunk').subscribe(function (chunkId, swarmId, originId) {
                thi$.Total_Recv_P2P += jsd.config.CHUNK_SIZE;
                //thi$.calc_avg(false);
            });

            radio('jsd_waste_p2p_chunk').subscribe(function (chunkId, swarmId, originId) {
                thi$.numOfP2PWasteChunks++;
                thi$.Total_Waste_P2P += jsd.config.CHUNK_SIZE;
            });

            radio('jsd_p2p_pending_chunk').subscribe(function (chunkId, swarmId, originId) {

            });

            radio('chunkAddedToBlockMap').subscribe(function () {
                thi$.Size_Uploaded_To_Memory += jsd.config.CHUNK_SIZE;
            });


            radio('chunkSentEvent').subscribe([function (blockMap) {
                thi$.Total_Sent_P2P += jsd.config.CHUNK_SIZE;
            }]);
